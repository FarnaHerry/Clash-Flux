...
- iOS CI 用固定 HuxerUI revision 编译 `clash-flux_huxerui_ios_core`，再用
  `platform/ios/clash-flux.xcodeproj` 构建 arm64 iOS Simulator `.app`，并将
  `clash-flux-VERSION-ios-simulator-arm64.zip` 纳入 Release 门禁和产物。此包仅供
  模拟器使用；实体设备 IPA、Network Extension、sing-box iOS 接入和 curl TLS 仍是 TODO。

...
Profile rowToProfile(SQLite::Statement& q) {
    ...
    p.nativeConfig = q.getColumn(15).getString();
    p.nativeRoutes = q.getColumn(16).getString();
    p.homepage = q.getColumn(17).getString();
    p.usedBytes = q.getColumn(18).getInt64();
    p.totalBytes = q.getColumn(19).getInt64();
    return p;
}
...
constexpr const char* kProfileColumns =
    ...
    " use_core_proxy, allow_invalid_cert, native_config, native_routes,"
...
std::int64_t Db::saveProfile(const Profile& p) {
    if (p.id == 0) {
        SQLite::Statement q(impl_->db,
            ...
            " native_config, native_routes, homepage, used_bytes, total_bytes)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        q.bind(1, p.name);
        q.bind(2, p.url);
        q.bind(3, p.file);
        ...
        q.bind(15, p.nativeConfig);
        q.bind(16, p.nativeRoutes);
        q.bind(17, p.homepage);
        q.bind(18, p.usedBytes);
        q.bind(19, p.totalBytes);
        q.exec();
